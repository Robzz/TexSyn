...
use image::*;
//use imageproc::definitions::VecBuffer;
...
#[derive(Debug, Clone)]
/// Describe a patch in the source image
struct Patch {
...
fn blit_rect(bottom: &mut RgbImage, top: &RgbImage, rect: &Rect, buf_coords: (u32, u32)) {
...
pub struct QuilterParams<'a, D> where D: 'a + Fn(&Rgb<u8>, &Rgb<u8>) -> f64 {
    pub size: (u32, u32),
    pub patch_size: u32,
    pub overlap: u32,
...
pub struct Quilter<'a, D> where D: 'a + Fn(&Rgb<u8>, &Rgb<u8>) -> f64 {
    source: RgbImage,
    buffer_opt: Option<RgbImage>,
    params: QuilterParams<'a, D>
}

impl<'a, D> Quilter<'a, D> where D: 'a + Fn(&Rgb<u8>, &Rgb<u8>) -> f64 {
    ...
    /// Compute the error between the left overlap area of the specified
    /// patch and the buffer.
    fn patch_left_error(&self, patch: &Patch,
                        buf_coords: (u32, u32)) -> f64
        where D: Fn(&Rgb<u8>, &Rgb<u8>) -> f64
    {
        patch_rect_error(self.params. distance_func, &self.source,
                         self.buffer_opt.as_ref().unwrap(), patch.coords,
                         buf_coords, (self.params.overlap, patch.size))
    }

    /// Compute the error between the top overlap area of the specified
    /// patch and the buffer.
    fn patch_top_error(&self, patch: &Patch, buf_coords: (u32, u32)) -> f64
        where D: Fn(&Rgb<u8>, &Rgb<u8>) -> f64
    {
        patch_rect_error(self.params.distance_func, &self.source,
                         self.buffer_opt.as_ref().unwrap(), patch.coords,
                         buf_coords, (patch.size, self.params.overlap))
    }

    /// Compute the error between the whole overlap area of the specified
    /// patch and the buffer.
    ...
    {
        match area {
            OverlapArea::Top => self.patch_top_error(patch, buf_coords),
            OverlapArea::Left => self.patch_left_error(patch, buf_coords),
            OverlapArea::TopLeft => self.patch_topleft_error(patch, buf_coords),
        }
    }
        ...
        where D: Fn(&Rgb<u8>, &Rgb<u8>) -> f64
    {
        ...
        let mut candidates_scores = vec!();
        let mut current_best = std::f64::INFINITY;
        let mut rng = thread_rng();
                    ...
                    for x in 0..max_x + 1 {
                        ...
                        if d > chance {
                            ...
                            if error < current_best * (1. + TOLERANCE) {
                                current_best = if error < current_best { error } else { current_best };
                                candidates_scores.push((p, error));
                            }
                        }
                    }
                }
        ...
        let mut candidates: Vec<Patch> = candidates_scores.into_iter().filter_map(|(p, err)| if err > current_best * (1. + TOLERANCE) { None } else { Some(p) }).collect();
    ...
    fn cut_and_blit_patch(&mut self, patch: &Patch, buf_coords: (u32, u32),
        ...
        match area {
            OverlapArea::Left => {
                ...
                let mut buffer = self.buffer_opt.as_mut().unwrap();
                blit_rect(&mut buffer, &self.source,
...
mod tests {
    ...
    fn test_patch_rect_error() {
        ...
        let f = patch_rect_error(&distance::l1, &i1, &i2, (4, 4), (0, 0), (3u32, 3u32));
